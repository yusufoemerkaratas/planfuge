...
def main() -> None:
    ...
    parser.add_argument(
        "--plan-id", default="SP_U1_0003", help="Plan ID to compare. Defaults to SP_U1_0003."
    )
    parser.add_argument("--image", help="Path to plan image. Defaults to data/pages/<plan_id>.png")
    parser.add_argument(
        "--examples-json",
    ...
    if not check_tesseract_availability():
        print(
            "Error: Tesseract is not available in PATH. Cannot perform comparison.", file=sys.stderr
        )
        sys.exit(1)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        baseline_candidates = run_png_extraction_pipeline(
            image_path=image_path, plan_id=plan_id, output_root=baseline_out, clean_red=False
        )

        print("\n--- Running Cleaned (clean_red=True) ---")
        cleaned_candidates = run_png_extraction_pipeline(
            image_path=image_path, plan_id=plan_id, output_root=output_root, clean_red=True
        )
    ...
    print("-" * 76)
    print(
        f"{'Total Detections / Crops':<40} | {len(baseline_candidates):<15} | {len(cleaned_candidates):<15}"
    )
    print(
        f"{'Parser Success Count':<40} | {baseline_parsed_count:<15} | {cleaned_parsed_count:<15}"
    )

    if examples:
        print(
            f"{'Relevant Examples Recalled':<40} | {baseline_metrics['matched_relevant']}/{baseline_metrics['total_relevant']} ({baseline_metrics['recall']:.2%}) | {cleaned_metrics['matched_relevant']}/{cleaned_metrics['total_relevant']} ({cleaned_metrics['recall']:.2%})"
        )
    ...
    for cand_id, cc in cleaned_map.items():
        ...
        if any(
            term in b_text or term in c_text for term in ["RA", "HSI", "RAS", "150", "190", "65"]
        ):
            found_target = True
            print(f"Candidate {cand_id}:")
            print(
                f"  [Baseline] Raw OCR: '{b_text.strip()}' -> Parsed: {bc.get('label_type')} (Width: {bc.get('width_mm')}, Height: {bc.get('height_mm')}, Reference: {bc.get('reference')}, RA: {bc.get('ra_value')})"
            )
            print(
                f"  [Cleaned]  Raw OCR: '{c_text.strip()}' -> Parsed: {cc.get('label_type')} (Width: {cc.get('width_mm')}, Height: {cc.get('height_mm')}, Reference: {cc.get('reference')}, RA: {cc.get('ra_value')})"
            )
            print()
