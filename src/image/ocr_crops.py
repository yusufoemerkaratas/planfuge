        ...
        with Image.open(crop_path) as img:
            ...
            if clean_red:
                try:
                    ...
                    logger.warning(
                        f"Red cleanup failed for {region_id}, falling back to original image: {cleanup_err}"
                    )
                    cleaned_img_rgb = img_rgb
            ...
            try:
                ...
                ocr_text = pytesseract.image_to_string(
                    img_gray, lang="deu+eng", config=config_str, timeout=5
                )
                used_lang = "deu+eng"
                ...
                try:
                    ocr_text = pytesseract.image_to_string(
                        img_gray, lang="eng", config=config_str, timeout=5
                    )
                    used_lang = "eng"
                    ...
                    try:
                        ocr_text = pytesseract.image_to_string(
                            img_gray, config=config_str, timeout=5
                        )
                        used_lang = None
            ...
            if clean_red and output_root is not None:
                try:
                    ...
                    result_txt_path = (
                        ...
                        / "debug"
                        / "ocr_cleanup"
                        / f"{plan_id}_{region_id}_result.txt"
                    )
            ...
            result["ocr_available"] = used_lang is not None or "OCR completely failed" not in (
                warning or ""
            )
...
def run_ocr_on_crops(
    crops_metadata: list, psm: int = 6, clean_red: bool = False, output_root: Path | None = None
) -> list:
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(
                _process_single_crop, item, psm, tesseract_available, clean_red, output_root
            )
