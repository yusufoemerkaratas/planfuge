...
for _, row in df_raw.iterrows():
    ...
    for m in pat_ddb_rnd.finditer(t):
        d = int(m.group(1))
        openings.append(
            {**base, "type": "Ceiling", "geometry": "round", "length_cm": d, "width_cm": d}
        )
        ...
        openings.append(
            {
                **base,
                ...
                "length_cm": int(m.group(1)),
                "width_cm": int(m.group(2)),
            }
        )
    ...
    for m in pat_wdb_rnd.finditer(t):
        d = int(m.group(1))
        openings.append(
            {**base, "type": "Wall", "geometry": "round", "length_cm": d, "width_cm": d}
        )
    ...
    for m in pat_uzdb_rnd.finditer(t):
        d = int(m.group(1))
        openings.append(
            {**base, "type": "UZDB", "geometry": "round", "length_cm": d, "width_cm": d}
        )
...
df_grouped = df.groupby(group_cols).size().reset_index(name="number")
